...
def separate_faces(image):
    # 7024 face selection rectangles
    rects = [(446, 771, 2810, 189),
             (446, 1107, 2810, 346),
             (446, 1516, 2810, 375),
             (446, 1910, 2810, 246)
             ]
...
def process_7024step2(image, drawable):
    ######################################
    # Manually Use Difference of Gaussians
    ######################################
    
    # Overlay the appropriate reference mask and subtract everything unnecessary
    # filename = "C:/Users/Ryan.Larson.ROCKWELLINC/github/color-deviation/masks/reference_masks/7024_ref_mask.png"
    config = load_config()
    filename = config.get("7024_ref_mask")
