# -*- coding: utf-8 -*-
"""
Created on Fri Nov  1 09:15:00 2024
...
def load_config():
    ...
    # config_path = "C:/Users/Ryan.Larson.ROCKWELLINC/github/color-deviation/gimp_plug_ins/config.json"
...
def separate_faces(image):
    # 6024 face selection rectangles
    rects = [(446, 719, 2810, 260),
             (446, 1123, 2810, 325),
             (446, 1542, 2810, 391),
             (446, 1941, 2810, 219)
             ]
    ...
    pdb.gimp_selection_none(image)
    # pdb.gimp_threshold(image.active_layer, 255, 255)
    
    pdb.gimp_layer_set_opacity(thermal_shock_layer, 0)
    # pdb.gimp_layer_set_opacity(image.active_layer, 5)
    
    # pdb.gimp_image_undo_group_end(image)
    
    

def process_6024step2(image, drawable):
    ######################################
    # Manually Use Difference of Gaussians
    ######################################
    
    # Overlay the appropriate reference mask and subtract everything unnecessary
    # filename = "C:/Users/Ryan.Larson.ROCKWELLINC/github/color-deviation/masks/reference_masks/6024_ref_mask.png"
    config = load_config()
    filename = config.get("6024_ref_mask")
